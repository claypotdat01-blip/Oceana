...
# =========================================
@st.cache_data
def prepare_spatial_data(df_input):
    """
    Menggunakan data koordinat asli dari CSV.
    Jika user memilih resolusi Musiman/Tahunan, nilai akan diagregasi
    (dirata-rata) berdasarkan titik spasialnya (lat, lon).
    """
    if df_input.empty:
        return pd.DataFrame()

    # Daftar parameter numerik yang akan dipetakan
    cols_to_agg = [
        'Ocean_Health_Index', 'Fisheries_Index', 'sst', 'ssta', 'ph',
        'do', 'salinitas', 'chla', 'current_speed', 'gelombang',
        'angin_u', 'angin_v', 'uo', 'vo'
    ]

    # Pastikan hanya diagregasi jika kolom eksis di dataframe
    avail_cols = [c for c in cols_to_agg if c in df_input.columns]

    # Rata-ratakan nilai berdasarkan titik koordinat
    df_spatial = df_input.groupby(['lat', 'lon'])[avail_cols].mean().reset_index()

    # Drop baris yang tidak memiliki nilai (membersihkan sisa masking daratan dari data mentah)
    df_spatial = df_spatial.dropna()

    return df_spatial

# Panggil fungsi menggunakan data yang sudah terfilter sesuai interaksi di Sidebar
df_map = prepare_spatial_data(df_filter_base)
